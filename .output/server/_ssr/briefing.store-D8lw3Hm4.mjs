...
import { i as fetchBlob, n as GATEWAY_URL, o as restCall, r as camelizeKeys } from "./http-DhyEQgDt.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/briefing.store-D8lw3Hm4.js
function emptyBriefingSummary() {
	return {
		category: {
			value: null,
			subValue: null,
...
*/
function summaryFromBrief(brief) {
	const category = brief["category"];
	const needType = brief["needType"];
	const subCategory = brief["subCategory"];
	...
	const budgetValue = budgetMin !== void 0 && budgetMin !== null ? budgetMax !== void 0 && budgetMax !== null ? `${budgetMin} € - ${budgetMax} €` : `${budgetMin} €` : null;
	return {
		category: {
			value: category ?? null,
		...
		budget: {
			value: budgetValue,
			done: budgetMin != null && budgetMax != null
		},
		location: {
			value: location ?? null,
			subValue: null,
		...
		deadline: {
			value: days !== void 0 && days !== null ? `${days} jour(s)` : null,
			subValue: null,
			done: days !== void 0 && days !== null
		}
	};
}
/** Étape (1-5, cf. `BriefingStepper`) courante + étapes complétées, dérivées du brief. */
function stepsFromSummary(summary) {
	const order = [
		{
			id: 1,
			done: summary.category.done
		},
		{
			id: 2,
			done: summary.description.done
		},
		{
			id: 3,
			done: summary.budget.done
		},
		{
			id: 4,
			done: summary.location.done
		},
		{
			id: 5,
			done: summary.deadline.done
		}
	];
	const completedSteps = order.filter((s) => s.done).map((s) => s.id);
	const firstUnfinished = order.find((s) => !s.done);
	return {
		step: firstUnfinished ? firstUnfinished.id : 5,
		completedSteps
	};
}
/**
...
*/
async function sendBriefingMessage(payload) {
	const raw = await restCall("ia", "/briefing/turn", {
		method: "POST",
		body: {
	...
	return {
		aiMessage: {
			id: `ai-${Date.now()}`,
			author: "ai",
			content: question ?? "Merci, votre brief est complet ! Vous pouvez le consulter dans le récapitulatif ci-contre.",
			time: nowLabel()
		},
		brief,
		summary,
...
*/
async function acceptSuggestion(_conversationId, _suggestionId) {
...
*/
async function generateCdcPdf(brief) {
	const clientEmail = useAuthStore.getState().user?.email ?? "";
	const raw = await restCall("ia", "/briefing/confirm", {
		method: "POST",
		body: {
			client: clientEmail,
			brief
...
var useBriefingStore = create()(persist((set, get) => ({
	...
	hasDraft: () => {
		const state = get();
		return state.conversationId !== null || state.conversationHistory.length > 0 || Object.keys(state.currentBrief).length > 0;
	},
	reset: () => set({
		...initialState,
		summary: emptyBriefingSummary()
...
//#endregion
export { useBriefingStore as i, generateCdcPdf as n, sendBriefingMessage as r, acceptSuggestion as t };
